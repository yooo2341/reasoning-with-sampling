...
if __name__ == "__main__":
    ...
    print("dataset done")
    if model == "phi":
        # Phi remote code uses past_key_values.seen_tokens, broken on transformers>=5
        config = transformers.AutoConfig.from_pretrained(model_str, trust_remote_code=False)
        tokenizer = transformers.AutoTokenizer.from_pretrained(model_str, trust_remote_code=False)
        hf_model = transformers.AutoModelForCausalLM.from_pretrained(
            model_str, config=config, torch_dtype="auto", device_map="auto", trust_remote_code=False
        ).to(device)
    else:
